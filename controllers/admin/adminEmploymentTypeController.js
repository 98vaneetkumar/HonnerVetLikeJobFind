...
module.exports = {
	save: async(payloadData) => {
		...
		let create = await Services.EmploymentTypeService.saveData(objToSave);
		if (create) {
	...
	getList: async(payloadData) => {
		...
		let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
		let result= {};
		result.data = await Services.EmploymentTypeService.getListing(
	...
	getDetail: async(payloadData) => {
		...
		let detail = await Services.EmploymentTypeService.getOne(criteria);
		return detail;
	},
	update: async(payloadData) => {
		...
		if (_.has(payloadData, "isDeleted") && payloadData.isDeleted != "") objToSave.isDeleted = payloadData.isDeleted;
		if (_.has(payloadData, "isBlocked") ) objToSave.isBlocked = payloadData.isBlocked;
		await Services.EmploymentTypeService.updateData(criteria, objToSave);
