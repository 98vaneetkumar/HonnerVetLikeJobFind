...
module.exports = {
	save: async(payloadData) => {
		...
		let create = await Services.JobTitleService.saveData(objToSave);
		if (create) {
	...
	getList: async(payloadData) => {
		...
		let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
		let result= {};
		result.data = await Services.JobTitleService.getListing(
			payload, parseInt(payload.limit, 10) || 10, parseInt(payload.skip, 10) || 0);
		result.count= 	await Services.JobTitleService.count(payload);
		return result;
	},
	getDetail: async(payloadData) => {
		...
		let detail = await Services.JobTitleService.getOne(criteria);
		return detail;
	},
	update: async(payloadData) => {
		...
		await Services.JobTitleService.updateData(criteria, objToSave);
