...
module.exports = {
	save: async (payloadData) => {
		try {
			...
			let create = await Services.SecurityClearanceService.saveData(objToSave);
			if (create) {
	...
	getList: async (payloadData) => {
		try {
			...
			let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
			let result = {};
			result.data = await Services.SecurityClearanceService.getListing(
				payload,
	...
	getDetail: async (payloadData) => {
		try {
			...
			let detail = await Services.SecurityClearanceService.getOne(criteria);
			return detail;
		} catch (err) {
			console.log(err);
			throw err;
	...
	update: async (payloadData) => {
		try {
			...
			await Services.SecurityClearanceService.updateData(criteria, objToSave);
