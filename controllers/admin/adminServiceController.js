...
module.exports = {
	save: async(payloadData) => {
		...
		let create = await Services.ServiceService.saveData(objToSave);
		if (create) {
	...
	getList: async(payloadData) => {
		...
		let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
		let result= {};
		result.data = await Services.ServiceService.getListing(
	...
	getDetail: async(payloadData) => {
		...
		let detail = await Services.ServiceService.getOne(criteria);
		return detail;
	},
	update: async(payloadData) => {
		...
		await Services.ServiceService.updateData(criteria, objToSave);
