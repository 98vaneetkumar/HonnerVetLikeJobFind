...
module.exports = {
	save: async (payloadData) => {
		try {
			...
			let create = await Services.SupplementService.saveData(objToSave);
			if (create) {
	...
	getList: async (payloadData) => {
		try {
			...
			let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
			let result = {};
			result.data = await Services.SupplementService.getListing(
				payload,
				parseInt(payload.limit, 10) || 10,
				parseInt(payload.skip, 10) || 0
			);
			result.count = await Services.SupplementService.count(payload);
			return result;
		} catch (err) {
			console.log(err);
			throw err;
	...
	getDetail: async (payloadData) => {
		try {
			...
			let detail = await Services.SupplementService.getOne(criteria);
			return detail;
		} catch (err) {
			console.log(err);
			throw err;
	...
	update: async (payloadData) => {
		try {
			...
			await Services.SupplementService.updateData(criteria, objToSave);
