...
module.exports = {
	save: async(payloadData) => {
		...
		let create = await Services.UniversityService.saveData(objToSave);
		if (create) {
	...
	getList: async(payloadData) => {
		...
		let payload = await commonHelper.verifyJoiSchema(payloadData, schema);
		let result= {};
		result.data = await Services.UniversityService.getListing(
	...
	getDetail: async(payloadData) => {
		...
		let detail = await Services.UniversityService.getOne(criteria);
		return detail;
	},
	update: async(payloadData) => {
		...
		await Services.UniversityService.updateData(criteria, objToSave);
